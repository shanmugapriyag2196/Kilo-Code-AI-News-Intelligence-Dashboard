...
export class DedupService {
    ...
    async clusterDuplicates(articles) {
        ...
        for (let i = 0; i < articleIds.length; i++) {
            for (let j = i + 1; j < articleIds.length; j++) {
                ...
                if (normUrl1 === normUrl2)
                    continue;
                const similarity = await this.ai.computeSimilarity(a1.title + ' ' + (a1.content || '').slice(0, 500), a2.title + ' ' + (a2.content || '').slice(0, 500));
