...
let storage = null;
let aiService = null;
let gdeltAdapter = null;
export function initNewsRoutes(config) {
...
export async function refreshNews(config) {
    const db = getDb(config);
    if (!storage)
        storage = new StorageService(db);
    if (!aiService)
    ...
    try {
        ...
        for (const raw of allRawArticles) {
            const existing = db.prepare('SELECT id FROM articles WHERE url = ?').get(raw.url);
            if (existing) {
                articlesDuplicated++;
                continue;
            }
...
async function updateDerivedData(config) {
    if (!storage)
        return;
