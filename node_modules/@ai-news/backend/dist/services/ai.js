...
export class AIService {
    ...
    async generateSummary(content, title) {
        if (!this.client || !content || content.length < 100)
            return null;
        try {
    ...
    async classifyArticle(title, content) {
        const fallback = deterministicCategory(title, content);
        if (!this.client || !content)
            return fallback;
        try {
            const response = await this.client.chat.completions.create({
                model: this.config.model,
    ...
    async computeTrendingScore(article) {
        let score = 0;
        const titleLower = article.title.toLowerCase();
        const contentLower = (article.content || '').toLowerCase();
        if (titleLower.includes('launch') || titleLower.includes('released') || titleLower.includes('announces'))
            score += 20;
        if (titleLower.includes('openai') || titleLower.includes('google') || titleLower.includes('anthropic') || titleLower.includes('meta'))
            score += 15;
        if (titleLower.includes('billion') || titleLower.includes('million') || titleLower.includes('funding') || titleLower.includes('acquisition'))
            score += 25;
        if (titleLower.includes('breakthrough') || titleLower.includes('first') || titleLower.includes('new model'))
            score += 20;
...
function deterministicCategory(title, content) {
...
function deterministicSimilarity(text1, text2) {
